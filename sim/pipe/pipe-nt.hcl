...
# f_pc
# We still want to make sure M_icode still checks for jump. However, since we shouldn't have checked !M_Cnd, I changed it to M_Cnd. Furthermore, when M_ifun != UNCOND it represents when a mis-prediction is when a conditional jump is executed. Also, we have M_valE is because we don't want to go to a new branch.

# f_predPC
# For f_predPC, I wanted to predict valC by adding a check for unconditionals. All else would predict valP. Since I checked for a jump in the first line, I removed it from the { IJXX, ICALL} since it was redundant. All else was valP.

# Execute
# I added IJXX to aluA and aluB to get E_valC and 0, which represents the values for 'valC + 0' as explained in the hint.

# Pipeline Register Control
# Since the logic for misprediction was simlar to f_pc, I updated the misprediction logic in D_bubble and E_bubble. Thus, I changed 'E_icode == IJXX && !e_Cnd' to 'E_icode == IJXX && e_Cnd && E_ifun != UNCOND'. 
...
word f_pc = [
	...
	M_icode == IJXX && M_Cnd && M_ifun != UNCOND : M_valE;
...
word aluA = [
	...
	E_icode in { IJXX, IIRMOVQ, IRMMOVQ, IMRMOVQ } : E_valC;
...
word aluB = [
	...
	E_icode in { IJXX, IRRMOVQ, IIRMOVQ } : 0;
	# Other instructions don't need ALU
];
